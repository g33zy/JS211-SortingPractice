...
const stringsToNumbs = strNums.map(element => Number(element))
...
//   const reducedNums = sumEvens.reduce((a, c) => a + c)
...
//   console.log(reducedNums)
  ...
  const weatherStates = weather.map(object => object.weather_state_name).filter((state, index, array) => array.indexOf(state) === index)
  ...
  const idealTemp = weather.find(minTemp => minTemp.min_temp === 15.915).id
